...
# CUDA_VISIBLE_DEVICES=${cudaNum} OMP_NUM_THREADS=30 
# # ###########################################################################################
...
CUDA_VISIBLE_DEVICES=0 OMP_NUM_THREADS=30 python -m model_part.retriever.col_bert.colbert.retrieve \
    --amp \
    --doc_maxlen 350 \
    --mask-punctuation \
    --bsize 256 \
...
# ######################################################################
python -m model_part.retriever.col_bert.colbert.beir_eval \
    --dataset ${dataset} \
    --split ${test_split} \
    --data_dir ${raw_data_dir}\
    --log_dir ${LOG_DIR} \
