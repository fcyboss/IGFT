...
####
cwd = os.getcwd()
if join(cwd, "model_part") not in sys.path:
    sys.path.append(join(cwd, "model_part"))
    sys.path.append(join(cwd, "data_part"))
from weak_data_loader import WeakDataLoader
data_dir = join(cwd, "model_part", "datasets")
raw_dir = join(data_dir, "raw")
weak_dir = join(data_dir, "weak")
beir_dir = join(raw_dir, "beir")
data_part_dir = join(cwd, "data_part", "data")

#### Download nfcorpus.zip dataset and unzip the dataset

parser = argparse.ArgumentParser()
parser.add_argument('--dataset_name', required=False, default="msmarco", type=str)
parser.add_argument('--num_epochs', required=False, default=40, type=int)
...
model_name ='/individual/tongzhenyu/model/bert-base-uncased'
...
if args.exp_name == "no_aug":
    corpus, queries, qrels = GenericDataLoader(corpus_file=join(beir_dir, args.dataset_name, f"corpus_{args.weak_num}_reduced_ratio_20.jsonl"), query_file=join(beir_dir, args.dataset_name, "queries.jsonl"), qrels_file=join(data_part_dir, f"{args.dataset_name}_{args.train_num}", f"prompt_tuning_{args.train_num}.tsv")).load_custom()
    print(type(qrels),len(qrels))
    print(type(corpus),len(corpus))
    print(type(queries),len(queries))
    queries = {key: queries[key] for key in random.sample(list(queries.keys()), 100)}
    print(len(qrels))
    print(len(queries))

else:
    # add support for loading weak data and ori train as new train
    weak_query_file = join(data_part_dir, f"{args.dataset_name}_{args.train_num}", args.weak_num, f"weak_queries_{args.train_num}_{args.exp_name}.jsonl")
    weak_qrels_file = join(data_part_dir, f"{args.dataset_name}_{args.train_num}", args.weak_num, f"weak_train_{args.train_num}_{args.exp_name}.tsv")
    corpus, queries, qrels = WeakDataLoader(corpus_file=join(beir_dir, args.dataset_name, f"corpus_{args.weak_num}_reduced_ratio_20.jsonl"), query_file=join(beir_dir, args.dataset_name, "queries.jsonl"), qrels_file=join(data_part_dir, f"{args.dataset_name}_{args.train_num}", f"prompt_tuning_{args.train_num}.tsv"), weak_query_file=weak_query_file, weak_qrels_file=weak_qrels_file).load_weak_custom()
...
# ir_evaluator = retriever.load_ir_evaluator(dev_corpus, dev_queries, dev_qrels,  score_functions=score_functions)
