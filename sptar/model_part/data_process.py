...
data_part_data_dir = join(cwd, "data_part", "data")
...
def sample_corpus(dataset_name, ratio: int = 20, train_num: int = 500):
    ...
    sampled_corpus_path = join(data_part_data_dir, f"{dataset_name}_{train_num}", f"corpus_filtered_5000_id.tsv")
    ...
    train_path = join(data_part_data_dir, f"{dataset_name}_{train_num}", f"prompt_tuning_{train_num}.tsv")
...
def sample_corpus_v2(dataset_name, ratio: int = 20, train_num: int = 50, weak_num: str = "100k"):
    ...
    sampled_corpus_path = join(data_part_data_dir, f"{dataset_name}_{train_num}", weak_num, f"corpus_filtered_{weak_num}_id.tsv")
    if os.path.exists(reduced_corpus_path):
        os.remove(reduced_corpus_path)
        print(f"old {reduced_corpus_path} is deleted ...")
    filtered_corpus = read_json(filtered_corpus_path)
